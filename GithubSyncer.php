...
class GithubSyncer
{
    ...
    protected static function http($url, &$headers, $post_params = null, $custom_method = null)
    {
        ...
        error_log('Remains: ' . $headers['X-RateLimit-Remaining'] . ' ' . $url);
        curl_close($curl);
        return $body;
    }
    ...
    public static function command_sync($params, $options)
    {
        ...
        if (!array_key_exists('dry-run', $options) and $way == 'upload') {
            // TODO: delete = true ?
            ...
            foreach ($uploaded as $filename => $sha) {
                $tree = array(
                    'path' => trim(implode('/', $path_terms) . '/' . $filename, '/'),
                    'mode' => sprintf('%o', fileperms($local_path . '/' . $filename)),
                    'type' => 'blob',
                    'sha' => $sha,
                );
            ...
            $ret = json_decode(self::http($url, $headers, json_encode(array(
                'base_tree' => $tree_sha,
                'tree' => array_values($trees),
            ))));

            if (!$ret->sha) {
                die ("Create tree failed: " . $ret->message);
            }
            $commit_tree = $ret->sha;
            ...
            $ret = json_decode(self::http($url, $headers, json_encode(array(
                'message' => array_key_exists('m', $options) ? strval($options['m'][0]) : 'Sync by github-syncer',
                'tree' => $commit_tree,
            ...
            var_dump($ret);
        } else if (!array_key_exists('dry-run', $options) and $way == 'download') {
        }
    ...
    public static function help($params, $options = null)
    {
        if (is_array($params)) {
        ...
        switch ($command) {
        case 'ls':
            echo "Usage: github-syncer ls <username>/[<repository>/[<path>]]\n";
            break;

        case 'sync':
            echo "Usage: github-syncer sync github://<username>/<repository>/[<path>] <local path>\n";
            echo "       github-syncer sync <local path> github://<username>/<repository>/[<path>]\n";
            echo "\n";
            echo "Optional arguments:\n";
            echo "-m <commit message> Commit with message, default: Sync by github-syncer\n";
            echo "-r                  Recursive sync (TODO)\n";
            echo "--delete            Delete nonexists file(TODO)\n";
            break;

        default:
            echo ("Usage: github-syncer <command> [<args>]\n");
            echo "\n";
            echo ("Command:\n");
            echo "ls       list file on github\n";
            echo "sync     sync file between local and github\n";
            echo "help     show help info\n";
            echo "Optional arguments:\n";
            echo "-u <username>     Login with <username> and will prompt to input password\n";
            echo "-t <oauth_token>  Login with Personal Access Token\n";
            break;
        }
    }
